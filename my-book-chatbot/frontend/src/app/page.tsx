...
export default function Home() {
  ...
  const [messages, setMessages] = useState<{id: string, content: string, role: string}[]>([
    { id: '1', content: 'Hello! I\'m your AI Tutor assistant. How can I help you with the documentation?', role: 'assistant' }
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      // Call backend API
      const response = await fetch('http://127.0.0.1:8000/chat/default_thread', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className={`min-h-screen font-sans ${darkMode ? 'bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900' : 'bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50'}`}>
      <main
        ref={mainRef}
      ...
      >
        ...
        <section className="flex-1 flex flex-col items-center justify-center px-4 py-12">
          <div className="max-w-4xl mx-auto text-center mb-12">
            <h2 className={`text-4xl md:text-6xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r ${
              darkMode
            ...
            <p className={`text-xl mb-8 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Get instant answers to your questions about robotics, AI, and documentation with our intelligent chatbot.
            </p>

            <div className={`inline-flex items-center space-x-2 px-6 py-3 rounded-full ${
              darkMode ? 'bg-gray-800 text-blue-300' : 'bg-blue-100 text-blue-700'
            }`}>
        ...
        {showHighlightConfirm && selectedText && (
          ...
          >
            <p className="text-sm mb-3">
              <strong>You selected:</strong> "{selectedText.substring(0, 60)}{selectedText.length > 60 ? '...' : ''}"
            </p>
        ...
        {chatVisible && (
          <div className={`fixed bottom-24 right-6 w-full max-w-sm h-[450px] z-50 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl border flex flex-col ${
            darkMode
          ...
          style={{ fontFamily: "'Exo 2', sans-serif" }}>
            ...
            <div className={`relative p-3 flex-shrink-0 ${
              ...
              <div className="flex items-center justify-between relative z-10">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <h3 className="font-bold text-sm" style={{ fontFamily: "'Orbitron', sans-serif" }}>🤖 AI Assistant</h3>
                  </div>
                </div>
                <button
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3 flex-grow" style={{ minHeight: 0, scrollbarWidth: 'thin', fontFamily: "'Exo 2', sans-serif" }}>
              {messages.map((msg) => (
                <div
                  key={msg.id}
              ...
              {isLoading && (
                <div className={`group relative max-w-[85%] mr-auto transition-all duration-300`}>
                  <div className={`p-3 rounded-xl shadow-lg backdrop-blur-sm border ${
                    darkMode
                      ? 'bg-gradient-to-r from-[#0A0A1A]/50 to-[#1A0B2E]/50 text-gray-100 rounded-bl-md border-[#00D4FF]/20'
                      : 'bg-gradient-to-r from-gray-100 to-gray-50 text-gray-800 rounded-bl-md border-gray-200'
            ...
            <div className={`p-3 border-t backdrop-blur-sm flex-shrink-0 ${
              darkMode ? 'border-[#00D4FF]/20 bg-[#0A0A1A]/50' : 'border-gray-200 bg-white/50'
            }`}>
